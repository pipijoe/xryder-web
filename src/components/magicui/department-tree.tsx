...
import {ChevronDown, ChevronRight, Diamond, FileIcon} from "lucide-react";
        ...
        return (
            ...
            <span onClick={() => handleExpand(value)}>
                 {children.length > 0 ? (expandedItems?.includes(value)
                     ...
                     : closeIcon ?? <ChevronRight/> ) : <Diamond className="size-2"/>}
            </span>
                    <span onClick={(e) => onClick ? onClick(value) : e.preventDefault()}>{element}</span>
                </AccordionPrimitive.Trigger>
...
const CollapseButton = forwardRef<HTMLButtonElement,
    {
        elements: TreeViewElement[];
        expandAll?: boolean;
